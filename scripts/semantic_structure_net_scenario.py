...
def make_model() -> dict[str, Any]:
    ...
    objects = [
        ...
        true_a,
        true_b,
        _terminal("source-term", "value", 116, 229, "source", "source"),
        _terminal("false-term", "value", 415, 189, "sink", "node-false"),
        _terminal(
            "true-a-term",
            "value",
            355,
    ...
    return {
        ...
        "vi": {
            ...
            "all_structure_frame_wires": [false_wire, true_wire_a, true_wire_b],
            "nets": [
                {
                    "id": "net-false",
                    "source_terminal_id": "source-term",
...
def _box_close(first: dict[str, float], second: dict[str, float]) -> bool:
    return all(abs(float(first[key]) - float(second[key])) <= 1 for key in first)
    ...
    return page.evaluate(
        """() => ({
          ...
          catalogWires: window.VIFrameWireCatalog
            ? window.VIFrameWireCatalog.allWires(window.VISemanticEditor.S.vi).map(wire => wire.id).sort()
            : [],
          ...
          box: window.VISemanticEditor.S.box ? {...window.VISemanticEditor.S.box} : null,
          dirtyFalse: window.VISemanticEditor.S.dirty.has('node-false'),
          localFalse: window.VISemanticEditor.S.local.get('node-false') || null,
          frameSectionVisible: Boolean(document.querySelector('#vi-structure-frame-section:not([hidden])')),
          netSectionVisible: Boolean(document.querySelector('#vi-net-inspector-section:not([hidden])')),
...
def _record_stage(
    ...
    print(
        "ISSUE14_STAGE="
        + json.dumps({"name": name, "state": value}, ensure_ascii=False, separators=(",", ":")),
        flush=True,
    )
    return value
...
def _open_editor(page: Page) -> None:
    ...
    page.wait_for_function(
        "() => Boolean(window.VIStructureNetWorkflow?.ready && window.VIFrameWireCatalog?.ready && window.VICanvasDensity?.ready)"
    )
...
def _audit(page: Page, diagnostics: dict[str, Any]) -> None:
    ...
    initial = _record_stage(page, diagnostics, "initial")
    ...
    _require(
        initial["allWires"] == ["wire-false", "wire-true-a", "wire-true-b"],
    ...
    true_frame = _record_stage(page, diagnostics, "true_frame")
    ...
    _require(true_frame["visibleTrueA"] and true_frame["visibleTrueB"], "True-frame nodes are incomplete", diagnostics)
    _require(
    ...
    _require(true_frame["dirtyFalse"] and true_frame["localFalse"], "inactive-frame edit was discarded", diagnostics)
    _require(_box_close(initial["box"], true_frame["box"]), "frame switch reset pan/zoom", diagnostics)
    ...
    print("ISSUE14_STAGE=" + json.dumps({"name": "net", "state": net_state}, separators=(",", ":")), flush=True)
    ...
    _require(net_state["sourceMarks"] >= 1 and net_state["sinkMarks"] >= 2, "net endpoints are not marked", diagnostics)
    ...
    diagnostics["focus_mode"] = page.evaluate("() => window.VICanvasDensity.runtime.currentMode")
    _require(diagnostics["focus_mode"] == "focus", "net focus did not use focus mode", diagnostics)
    ...
    restored = _record_stage(page, diagnostics, "restored")
    ...
    _require(front["surface"] == "front-panel" and front["visible"], "front panel was damaged", diagnostics)
...
def main() -> int:
    ...
    with app_log_path.open("wb") as app_log:
        ...
        try:
            ...
            with sync_playwright() as playwright:
                ...
                _install_route(page, model)
                _open_editor(page)
                try:
                    _audit(page, diagnostics)
                except PlaywrightTimeoutError as error:
                    diagnostics["failures"].append(f"timeout during {diagnostics['stage']}: {error}")
    ...
    print(
        "SEMANTIC_STRUCTURE_NET_TEST_FAILED" if failed else "SEMANTIC_STRUCTURE_NET_TEST_OK",
        flush=True,
    )
