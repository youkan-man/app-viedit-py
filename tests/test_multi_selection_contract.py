...
def test_ctrl_shift_and_marquee_selection_paths_are_available() -> None:
    script = read("vi-editor-multi-selection.js")

    assert "event.ctrlKey || event.metaKey" in script
    assert "event.shiftKey && listButton" in script
    assert "selectListRange" in script
    assert "vi-multi-selection-marquee" in script
    assert "beginMarquee" in script
    assert "moveMarquee" in script
    assert "finishMarquee" in script
    assert "centerX >= rect.left" in script
    assert "centerY <= rect.bottom" in script
