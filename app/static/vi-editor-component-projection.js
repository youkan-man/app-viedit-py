...
(() => {
  ...
  function logicalBounds(item, index = 0) {
    const E = editor();
    return E?.effectiveBounds?.(item, index) || E?.getBounds?.(item, index) || null;
  }
  ...
  function factorFor(item) {
    ...
    const surface = item.surface === 'block-diagram-inactive'
      ? 'block-diagram'
      : item.surface;
    const policy = SURFACE_FACTORS[surface] || SURFACE_FACTORS['block-diagram'];
    if (item.category === 'terminal') return policy.terminal;
    const kind = normalizedKind(item);
    if (surface === 'front-panel') {
      for (const key of ('boolean', 'numeric', 'string', 'path', 'ring', 'table')) {
        if (kind.includes(key)) return policy[key];
      }
      return policy.default;
    }
    if (kind.includes('primitive') || kind.includes('add') || kind.includes('operator')) {
      return policy.primitive;
    }
    if (kind.includes('constant')) return policy.constant;
    if (kind.includes('subvi') || kind.includes('sub-vi')) return policy.subvi;
    return policy.default;
  }
  ...
  function projectNonTerminal(item, logical) {
    ...
    if (explicit) {
      const absolute = explicit.x >= logical.x - 1
        && explicit.y >= logical.y - 1
        && explicit.x <= logical.x + logical.width + 1
        && explicit.y <= logical.y + logical.height + 1;
      const x = absolute ? explicit.x : logical.x + explicit.x;
      const y = absolute ? explicit.y : logical.y + explicit.y;
      return {
        x,
        y,
    ...
    return {
      x: logical.x,
      y: logical.y,
      width: Math.max(4, logical.width * factor),
      height: Math.max(4, logical.height * factor),
      factor_x: factor,
      factor_y: factor,
  ...
  function projectTerminal(item, logical, stack) {
    ...
    const owner = parentId && !stack.has(parentId) ? S?.objects.get(parentId) : null;
    if (owner) {
      ...
      if (ownerLogical) {
        ...
        centerX = ownerProjected.x + ownerProjected.width * ratioX;
        centerY = ownerProjected.y + ownerProjected.height * ratioY;
      }
    }
    const width = Math.max(7, logical.width * factor);
    const height = Math.max(7, logical.height * factor);
    return {
      x: centerX - width / 2,
      y: centerY - height / 2,
      width,
      ...
      source: parentId ? 'owner-relative-terminal' : 'terminal-center',
    };
  }
  ...
  function projectBounds(item, suppliedBounds = null, stack = new Set()) {
    ...
    if (item.category === 'terminal') return projectTerminal(item, logical, stack);
  ...
  function projectedWirePoints(wire, targetTerminalId, targetObjectId) {
    ...
    let bends = orient(cleanPoints(wire), source, target);
    if (bends.length && distance(bends[0], source) < 18) bends.shift();
    if (bends.length && distance(bends.at(-1), target) < 18) bends.pop();
    if (!bends.length) {
      const middle = source.x + (target.x - source.x) / 2;
      bends = [{ x: middle, y: source.y }, { x: middle, y: target.y }];
    }
  ...
  function applyWireProjection(group) {
    ...
    if (!wire) return;
    const branchIndex = Math.max(0, Number.parseInt(group.dataset.branchIndex || '0', 10));
    ...
    if (!path) return;
    group.querySelectorAll(':scope > .vi-wire,:scope > .vi-wire-hit').forEach((element) => {
  ...
  function clientToWorld(event) {
    const svg = state()?.el?.modelGraphSvg;
  ...
  function moveResize(event) {
    ...
    const width = Math.max(
      8,
      gesture.logical.width + displayDeltaX / Math.max(0.05, gesture.factorX),
    );
    const height = Math.max(
      8,
      gesture.logical.height + displayDeltaY / Math.max(0.05, gesture.factorY),
    );
  ...
  function beginResize(event) {
    ...
    const projected = projectBounds(item, logical);
    const world = clientToWorld(event);
    if (!handle || !group || !item || !logical || !projected || !world) return;
    event.preventDefault();
    event.stopImmediatePropagation();
  ...
  function projectedMetrics() {
    const root = state()?.el?.modelGraphSvg;
    const records = [...(root?.querySelectorAll('[data-object-id]') || [])].map((group) => ({
    ...
    return {
      ...
      projected: records.filter((record) => record.scaleX < 0.999 || record.scaleY < 0.999).length,
      records,
    };
  }
  ...
  function install() {
    ...
    decorate();
    requestAnimationFrame(() => globalThis.VICanvasDensity?.fit?.('readable'));
    return true;
  }
  ...
  globalThis.VIComponentProjection = { ready: false, runtime, SURFACE_FACTORS };
  waitForEditor();
})();
