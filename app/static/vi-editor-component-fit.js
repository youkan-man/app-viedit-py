...
(() => {
  ...
  function wirePoints() {
    ...
    return [...S.wires.values()].flatMap((wire) => {
      if (project?.projectedWirePoints) {
        return (wire.target_terminal_ids || []).flatMap(
          (terminalId, index) => project.projectedWirePoints(
            wire,
            terminalId,
  ...
  function representativeMetrics() {
    ...
    return {
      ...
      medianCenterX: median(
        records.map((record) => record.x + record.width / 2),
      ),
      medianCenterY: median(
        records.map((record) => record.y + record.height / 2),
      ),
    };
  }
  ...
  function fit(mode = 'readable') {
    ...
    if (applied && state() && mode === 'readable') {
